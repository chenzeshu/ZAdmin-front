<template>
    <div class="v-services">
        <!--添加按钮-->
        <div class="service-plus" @click="confirmNew()">
        ...
        <Modal  v-model="modalNew" width="400" title="新增服务单"
            ...
            <br>
            <Input v-model="singleNew.time4" placeholder="占用工时（天）" style="width: 300px">
                    ...
                    },
                    {
                        ...
                        render(row,columns,index){
                            switch (row.result_deal){
                                case 0:
                                    return `<i-button size="small">待解决</i-button>`
                                    break;
                                case 1:
                                    return `<i-button type="success" size="small" @click="showDeal(${index})">已解决</i-button>`
                                    break;
                                case 2:
                                    return `<i-button type="warning" size="small" @click="showDeal(${index})">未解决</i-button>`
                                    break;
                                default:
                                    break;
                            }
        ...
        methods:{
            ...
            print(index){
                ...
                this.$http.post('services/createTask', n).then(res=>{
                    this.$Loading.finish();
//                    location.href = "http://lv.app/api/v1/services/printTask"
                    location.href = base.url + "services/printTask"
                }, err=>{
                    this.$Loading.error();
                    this.$Message.error('导出失 败')
                })
            },
            confirmNew(){
...
//                    err.status
            ...
            add(){
                ...
                this.$http.post('services', n).then(res=>{
                    ...
                    this.addRefresh();
                    location.reload()
//                    n.s_id = n.s_id===null ? "自动生成编号请刷新查看" : n.s_id
//                    n.time1 = unixToDate(Date.parse(new Date(n.time1))/1000, 'yyyy-MM-dd')
//                    n.time2 = n.time2 ? unixToDate(Date.parse(new Date(n.time2))/1000, 'yyyy-MM-dd'):null
//                    n.time3 = n.time3 ? unixToDate(Date.parse(new Date(n.time3))/1000, 'yyyy-MM-dd'):null
//                    n.type = parseInt(n.type)
//                    n.contract_type = parseInt(n.contract_type)
//                    n.charge_if = parseInt(n.charge_if)
//                    n.result_deal = parseInt(n.result_deal)
...
//                    this.services.unshift(n)
        ...
        mounted(){
            ...
            this.$http.get('services/'+this.contractId).then(res=>{
                this.$Loading.finish();
                ...
                let bread = {
                    ...
                    second : {
                        name:  this.services[0].contract.name + " 服务记录",
                        href : '#'
                    },
                    third : {}
                }
