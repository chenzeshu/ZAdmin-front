        ...
        methods:{
            handleSuccess(res, file, fileList){
               //this.$refs.upload.fileList每次上传，组件会自动this.$refs.upload.fileList.push(file),所以工作只有一个：编辑file的属性
                file.path = res.path
                this.index = null
                this.$Message.success('注意：编辑状态下，新增文件后，若要原有文件，应重新打开编辑界面')
            },
            handleError(err, file, fileList){
                $this.$Message.error(file.name+"上传失败")
            },
            handleRemove(file, fileList){
                //todo 新增时，删文件不动数据库；编辑时则修改。
                //todo 如果修改时，增加了新文件，这个文件在提交前不会被存到数据库（和新增一样），所以要再新增处(handleSuccess)重置index
                let _index = this.index!==null ? this.index : null
                let id; // 合同id
                if(_index!==null){
                    id = this._contracts[_index].id
                }else{
                    id = null
                }

                this.$http.post('contracts/unUpload/', {id: id, name:file.name, path:file.path}).then(res=>{
