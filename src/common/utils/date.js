export const compareDate = function (time1) {
    let time2 = Date.parse(new Date())/1000
    let min = time1 - time2

    if( min > 2073600){
        return 1 //未过期
    }else if ( 2073600 > min && min >= 0){
        return 2 //即将过期
    }else if (min <0){
        return 3 //已过期
    }
...
export const unixToDate = function (unixnumber,fmt) {
    let date = new Date(parseInt(unixnumber+"000"))
